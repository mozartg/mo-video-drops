...
| RT-005 | GitHub returned no CI statuses, and mergeability changed between `true` and `false` across successive reads. | Preserved as volatile provider metadata; CI remains untested and mergeability must be refreshed immediately before any merge. |
